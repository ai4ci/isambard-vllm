...
`.trim());
  ...
  if (entries.length === 0) {
    ...
    console.log("  First run: ivllm start <job-name> --config <path-to-vllm.yaml>");
    return;
  }
  ...
  const nameWidth = Math.max("JOB".length, ...entries.map((e) => e.jobName.length));
  const modelWidth = Math.max("MODEL".length, ...entries.map((e) => (e.model || "").length));
  ...
  for (const entry of entries) {
    ...
    console.log(`${entry.jobName.padEnd(nameWidth)}  ${model.padEnd(modelWidth)}  ${tp.padStart(2)}  ${pp.padStart(2)}`);
  }
}
