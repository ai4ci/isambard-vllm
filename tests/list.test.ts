...
describe("listJobConfigs", () => {
  ...
  it("lists stored configs with parsed metadata", () => {
    writeTestConfig("ivllm-test-single", `
...
`);
  ...
  it("ignores non-yaml files in the config directory", () => {
    if (!existsSync(JOB_CONFIG_DIR)) mkdirSync(JOB_CONFIG_DIR, { recursive: true });
    writeFileSync(join(JOB_CONFIG_DIR, "ivllm-test-config.json"), "{}", "utf-8");
    writeFileSync(join(JOB_CONFIG_DIR, "ivllm-test-readme.txt"), "hello", "utf-8");
