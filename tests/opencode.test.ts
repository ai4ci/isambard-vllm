...
import { formatOpencodeSnippet } from '../src/assistant.ts';
...
describe('formatOpencodeSnippet', () => {
  ...
  it('sets baseURL using localPort', () => {
    const out = JSON.parse(formatOpencodeSnippet(base));
    expect(out.provider['isambard-vllm'].options.baseURL).toBe(
  ...
  it('includes the model in the models map', () => {
    const out = JSON.parse(formatOpencodeSnippet(base));
    expect(
      out.provider['isambard-vllm'].models['google/gemma-4-31B-it'],
    ).toBeDefined();
  });
  ...
  it('sets the model display name with (Isambard) suffix', () => {
    const out = JSON.parse(formatOpencodeSnippet(base));
    expect(
      out.provider['isambard-vllm'].models['google/gemma-4-31B-it'].name,
  ...
  it('sets context limit from maxModelLen', () => {
    const out = JSON.parse(formatOpencodeSnippet(base));
    expect(
      out.provider['isambard-vllm'].models['google/gemma-4-31B-it'].limit
        .context,
    ).toBe(262144);
  });
  ...
  it('defaults context/output to 4096 when maxModelLen is not provided', () => {
    const out = JSON.parse(
      formatOpencodeSnippet({ model: 'some/model', localPort: 11434 }),
    );
  ...
  it('includes tool_call: true when toolCall is true', () => {
    ...
    expect(
      out.provider['isambard-vllm'].models['google/gemma-4-31B-it'].tool_call,
    ).toBe(true);
  });
  ...
  it('includes reasoning: true when reasoning is true', () => {
    ...
    expect(
      out.provider['isambard-vllm'].models['google/gemma-4-31B-it'].reasoning,
    ).toBe(true);
  });

  it('omits reasoning when reasoning is false', () => {
    const out = JSON.parse(
      formatOpencodeSnippet({ ...base, reasoning: false }),
    );
    expect(
      out.provider['isambard-vllm'].models['google/gemma-4-31B-it'].reasoning,
    ).toBeUndefined();
  ...
  it('produces valid JSON', () => {
    expect(() =>
      JSON.parse(
        formatOpencodeSnippet({ ...base, toolCall: true, reasoning: true }),
      ),
    ).not.toThrow();
  });
});
